...
- **ログ1件＝Drive上の1ファイル**: レコード単位の差分同期・競合解決を可能にするため。全件を1つの JSON にまとめる方式は、1レコードの変更でも全件再アップロードが必要になり同時編集時に全件が競合するため不採用。
...
- 墓標を `diveLogs.deletedAt` として持たせる方式は、`useDiveLogs()` の `orderBy('date')` 等の既存読み取り経路すべてに除外条件が要るため不採用。**別テーブル `tombstones`** とし、既存の読み取りコードは無変更で済む形にする。
...
- **アカウント表示**（REQ-1.8）: `GET /drive/v3/about?fields=user(displayName,emailAddress)`。`drive.file` スコープのみで取得できるかは実機検証が必要（検証項目）。不可の場合は `openid email` スコープ追加、または表示省略を検討。
- **設定**: `VITE_GOOGLE_CLIENT_ID` 未設定のビルドでは同期UIを非表示（REQ-1.9）。OAuth クライアントの「承認済みの JavaScript 生成元」に `https://plusnid.github.io` を登録（配信先は GitHub Pages サブパスに決定済み。[mobile-compatibility/design.md「配信先の反映」](../mobile-compatibility/design.md#配信先の反映)）。生成元はオリジン単位のためサブパス自体の登録は不要。
...
自動同期のトリガ（REQ-3.2）:
- 起動時: `App` マウント時、`enabled && autoSync && navigator.onLine` なら実行。
- オンライン復帰: `window.addEventListener('online', ...)`。
- 書き込み後: `createDiveLog` / `updateDiveLog` / `deleteDiveLog` 完了後のイベントを受け、5秒デバウンス後に実行。`db/` は変更通知（軽量な EventTarget等）のみ提供し、購読は `sync/syncEngine` 側で行う（リポジトリを `sync` に直接依存させない）。
...
- **時計ずれ**: 競合時の勝者判定は端末時計（`updatedAt`）に依存するため、ずれていると意図しない側が勝つ可能性がある。ただし敗者は競合コピーとして残るため記録自体は失われない。
- **アクセストークンの寿命**: リフレッシュトークンを持てない構成のため、リロード毎に無操作でのトークン再取得が必要（詳細は「認証設計」）。Google セッション切れ時はユーザー操作が必要（REQ-1.7）。
- **OAuth 同意画面が「テスト」モード**: Google 審査未対応のため同意の有効期間が短くなる場合があり、無操作再取得（`prompt: ''`）が失敗して再同意（`prompt: 'consent'`）が想定より高頻度になり得る。個人利用（登録済み本人のみ）前提のため許容する。
- **初回同期のデータ量／Drive の無料枠**: 写真を含むため既存ログが多い端末では初回同期に時間・通信量を要し、15GB の無料枠（Gmail 等と共有）を圧迫し得る。Wi-Fi 限定オプションや事前の使用量警告は設けない方針で確定済み（[要件「決定事項」](./requirements.md)）。容量不足は REQ-7.4 のエラーで事後的に気づく。
- **Drive 上での手動操作**: ユーザーが Drive 側でファイルを消す・移動する操作は想定しない。手動削除時はローカル内容で作り直す（決定表 #2、REQ-4.7）。
- **iOS のスタンドアロン起動でのポップアップ**: OAuth 同意ポップアップが正しく戻るかは実機検証が必要（[要件「検証タスク」](./requirements.md)）。問題時のリダイレクト方式へのフォールバックは検証後に判断し、現設計には含めない。
- **API クォータ**: 大量ログの一括同期でレート制限に達し得る（並列数制限とバックオフで緩和。「Drive クライアント」参照）。
- **テスト**: 自動テスト基盤がないため決定表（上記）の検証手段は手動確認のみ。実装と合わせたユニットテスト導入の検討を推奨する。
